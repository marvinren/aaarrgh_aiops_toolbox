...
class DrainLogParser(BaseLogParser):
    def __init__(self, log_format, outdir='./result/', depth=4, st=0.4,
                 maxChild=100, rex=[], keep_para=True):
        """
        使用Drain算法对日志进行聚类，需要构建前缀树，第一层是长度，第二层开始是前缀，叶子节点为日志模板
        depth: 前缀树的深度
        st: 相似度阈值
        maxChild: 每个节点的最大子节点数
        rex: 正则表达式，用于提取日志中的关键信息
